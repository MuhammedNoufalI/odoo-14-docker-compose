...
RESIGNATION_TYPE = [
    ...
    ('termination_lieu_res', 'Termination In Lieu of Resignation'),
    ('termination_article44', 'Termination Article (44)')
]
...
class HrResignation(models.Model):
    ...
    resign_confirm_date = fields.Date(string="Resignation Confirmed Date",
    ...
    expected_revealing_date = fields.Date(string=" Employee Exit Date", required=True,
    ...
    notice_period = fields.Char(string="Notice Period",requird=True,default=30)
    ...
    last_worked_days_monthly = fields.Float("Last Worked Days")
                ...
                if rec.employee_id:
                    ...
                    for contracts in no_of_contract:
                        if contracts.state == 'open':
                            ...
                            rec.notice_period = contracts.notice_days.name
            ...
            if rec.expected_revealing_date and rec.resign_confirm_date:
                ...
                for contracts in no_of_contract:
                    if contracts.state == 'open':
                        rec.employee_contract = contracts.name
                        rec.state = 'approved'
                        rec.approved_revealing_date = rec.resign_confirm_date + timedelta(
                            days=int(contracts.notice_days.name))
                    else:
    ...
    resign_date = fields.Date(string="Resignation Date", )
    total_days = fields.Integer(string="Total Days", compute='_get_total_days')

    @api.onchange('notice_period', 'resign_date')
    def _get_exit_date(self):
        for rec in self:
            if rec.resign_date and rec.notice_period:
                rec.expected_revealing_date = rec.resign_date + timedelta(days=int(rec.notice_period))

    @api.onchange("employee_id", 'joined_date', 'resign_date')
    def _get_total_days(self):
        for rec in self:
            if rec.joined_date and rec.expected_revealing_date:
                d1 = date(rec.expected_revealing_date.year, rec.expected_revealing_date.month,
                          rec.expected_revealing_date.day)
                d2 = date(rec.joined_date.year, rec.joined_date.month, rec.joined_date.day)
                delta = (d1 - d2)

                rec.total_days = delta.days
            else:
                rec.total_days = False
