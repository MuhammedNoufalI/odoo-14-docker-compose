...
class HrPayslip(models.Model):
    ...
    @api.depends('expense_sheet_ids', 'employee_id', 'date_from', 'date_to')
    def _compute_input_line_ids(self):
        """This Compute the other inputs to employee payslip."""
        res = super(HrPayslip, self)._compute_input_line_ids()
        for slip in self:
            slip.input_line_ids = None
            input_type_id = self.env['hr.payslip.input.type'].search([('code', '=', 'LOAN')])
            emp_id = slip.employee_id
            lon_obj = self.env['hr.loan'].search([('employee_id', '=', emp_id.id), ('state', '=', 'approve')])
            for loan in lon_obj:
                for loan_line in loan.loan_lines:
                    if slip.struct_id and slip.date_from and slip.date_to and slip.date_from <= loan_line.date <= slip.date_to and not loan_line.paid:
                        if loan_line not in slip.input_line_ids.mapped('loan_line_id') and input_type_id:
