...
class ImportPicking(models.TransientModel):
    ...
    def make_picking_line(self, values, pick_id):
        ...
        expiry_date = False
        lot_id = False
        if values.get('default_code')[-2:] == '.0':
            float_code = float(values.get('default_code'))
            int_code = int(float_code)
            str_code = str(int_code)
        else:
            str_code = values.get('default_code')
        print("default_code", str_code)
        product_id = product_obj.search(
            [('default_code', '=', str_code)])
