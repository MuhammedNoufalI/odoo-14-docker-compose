    ...
    def get_data(self, start, end):
        ...
        for slip in slips:
            if ids:
                ...
                ids = ids + str(slip.id)
        query = """select hr_employee.id,labour_card_number, agent_id, hr_payslip_line.amount 
...
where hr_payslip_line.name = 'Net Salary' and hr_payslip_line.slip_id in("""+ids+""")"""
    ...
    def get_days(self, emp_id, start, end):
        slip = self.env['hr.payslip'].search(['&', ('employee_id', '=', emp_id)
                                                 , ('date_from', '=', start)
                                                 , ('date_to', '=', end)],limit=1)
        days_records = self.env['hr.payslip.worked_days'].search([('payslip_id', '=', slip.id)])
        days = 0
        for rec in days_records:
