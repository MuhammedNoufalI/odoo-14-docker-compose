	...
	def import_img(self):
		...
		for img in zf.namelist()[1:]:
			...
			if self.img_for == 'product':
				...
				if self.emp_by == 'name':
					record = self.env['hr.employee'].search([('name','like',file)])
				if self.emp_by == 'id no':
					record = self.env['hr.employee'].search([('registration_number','=',file)])
				if self.emp_by == 'id':
					try:
						int(file)
