...
class Action(Action):
    ...
    def load(self, action_id, additional_context=None):
        ...
        if res:
            ...
            for view_data in set(request.env['remove.action'].sudo().search(
                    [('view_data_ids', '!=', False), ('access_management_id.company_ids', 'in', int(cids)),
                     ('access_management_id', 'in', request.env.user.access_management_ids.ids),
    ...
    def web_client(self, s_action=None, **kw):
        ...
        if not kw.get('debug') or kw.get('debug') != "0":
            ...
            access_management = request.env['access.management'].sudo().search(
                [('active', '=', True), ('company_ids', 'in', int(cids)), ('disable_debug_mode', '=', True),
