...
class access_management(models.Model):
    ...
    user_ids = fields.Many2many('res.users', 'access_management_users_rel_ah', 'access_management_id',
                                'user_id', 'Users')

    readonly = fields.Boolean('Read-Only')
    active = fields.Boolean('Active', default=True)

    hide_menu_ids = fields.Many2many('ir.ui.menu', 'access_management_menu_rel_ah',
    ...
    def create(self, vals):
        """Raise UserError while creating the record if the field
        'readonly' is enabled for the admin users"""
    ...
    def write(self, vals):
        """Raise UserError while saving the record if the field
        'readonly' is enabled for the admin users"""
