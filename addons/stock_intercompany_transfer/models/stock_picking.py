...
class StockMoveInherit(models.Model):
    ...
    main_cost = fields.Float(string='Main cost')
        ...
        for rec in self:
            if rec.move_ids_without_package:
                for move in rec.move_ids_without_package:
                    if move.main_cost > 0:
                        move.product_id.standard_price = move.main_cost
        res = super(StockPickingInherit, self).button_validate()
        print("\n\n\n\n\nres : ", res)
        print("self : ", self)
        for rec in self:
            if not rec.auto_generated:
                rec.create_intercompany_transfer()
        return res
        ...
        if company_id and company_id.enable_inter_company_transfer:
            ...
            if create_transfer:
                ...
                for move in self.move_lines:
                    move.main_cost = move.product_id.standard_price
